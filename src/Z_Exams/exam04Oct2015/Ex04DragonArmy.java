...
public class Ex04DragonArmy {
    ...
    private static Function<TreeMap<String, Dragon>, String> getStats = dragon -> {
        double damage = dragon.values().stream().mapToDouble(Dragon::getDamage).average().getAsDouble();
        double health = dragon.values().stream().mapToDouble(Dragon::getHealth).average().getAsDouble();
        double armor = dragon.values().stream().mapToDouble(Dragon::getArmor).average().getAsDouble();
